		...
		var runInContext = function runInContext(context) {
			...
			function baseExtremum(array, iteratee, comparator) {
				...
				while (++index < length) {
					...
					if (
						current != null &&
						(computed === undefined$1 ? current === current && !isSymbol(current) : comparator(current, computed))
					) {
			...
			function createWrap(func, bitmask, thisArg, partials, holders, argPos, ary, arity) {
				...
				holders = newData[4];
				arity = newData[9] =
					newData[9] === undefined$1 ? (isBindKey ? 0 : func.length) : nativeMax(newData[9] - length, 0);
	...
	constructor(hass, plex, entity) {
		...
		this.isPlexPlayerSupported = () => {
			...
			if (!found) {
				lodash.forEach(this.plex.clients, plexClient => {
					if (
						lodash.isEqual(plexClient.product, this.entity.plexPlayer) ||
...
const getOffset = el => {
	let x = 0;
	let y = 0;
	while (el && el.offsetParent && !lodash.isNaN(el.offsetLeft) && !lodash.isNaN(el.offsetTop)) {
 ...
 */
window.trustedTypes && trustedTypes.createPolicy('lit-html', { createHTML: s => s });
