...
class PlayController {
    constructor(card, hass, plex, entity, runBefore, runAfter, libraryName) {
        ...
        this.getKodiTVShowSeason = async (tvshowID, seasonNumber) => {
            ...
            const results = await this.getKodiSearchResults();
            lodash.forEach(results, result => {
                if (lodash.isEqual(result.season, seasonNumber)) {
                    foundResult = result;
                    return false;
                }
            });
        ...
        this.play = async (data, instantPlay = false) => {
            if (lodash.isArray(this.runBefore)) {
                ...
                if (lodash.isEqual(entityState.state, 'on')) {
        ...
        this.playViaKodi = async (entityName, data, type) => {
            if (type === 'epg') {
                ...
                let foundEpisode = {};
                lodash.forEach(episodesData.episodes, episodeData => {
                    if (lodash.isEqual(episodeData.episode, data.index)) {
                        foundEpisode = episodeData;
                        return false;
                    }
                });
                if (lodash.isEmpty(foundEpisode)) {
                    throw Error(`Episode ${data.index} not found in Kodi for TV Show ${data.grandparentTitle} (id: ${kodiData.tvshowid}) season ${data.parentIndex}.`);
                }
                await this.hass.callService('kodi', 'call_method', {
                    // eslint-disable-next-line @typescript-eslint/camelcase
                    entity_id: entityName,
    ...
    constructor() {
        ...
        this.renderPage = () => {
            ...
            if (!this.card) {
                ...
                if (!lodash.isNil(this.config.title) && !lodash.isEmpty(this.config.title)) {
                    ...
                    if (this.detailElem && this.plex) {
                        ...
                        if (genreElem.parentElement) {
                            if (mainData.Genre && mainData.Genre.length > 0) {
                                let genre = '';
                                lodash.forEach(mainData.Genre, tag => {
                                    genre += `${tag.tag}, `;
                                });
                ...
                if (!lodash.isEmpty(seasonsData)) {
                    ...
                    lodash.forEach(seasonsData, seasonData => {
                        if (this.seasonsElem && this.plex) {
                            ...
                            const thumbURL = `${this.plex.getBasicURL()}/photo/:/transcode?width=${this.minExpandedWidth}&height=${this.minExpandedHeight}&minSize=1&upscale=1&url=${seasonData.thumb}&X-Plex-Token=${this.config.token}`;
                            ...
                            if (this.playController) {
                                const playButton = this.playController.getPlayButton(seasonData.type);
                                if (this.playController.isPlaySupported(seasonData)) {
                                    playButton.classList.remove('disabled');
                                }
